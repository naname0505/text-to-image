...
def main():
    ...
    print(args.caption_thought_vectors)
    ...
    for cn, caption_vector in enumerate(caption_vectors):
        ...
        print("Generated", cn)
        _val_sample_path = join(args.data_dir,'val_samples/',args.data_set,"/")
    
    """
    for f in os.listdir(_val_sample_path):
        if os.path.isfile(f):
            os.unlink(join(_val_sample_path + f))
    """
    ...
    for cn in range(0, len(caption_vectors)):
        ...
        scipy.misc.imsave( join(args.data_dir,'val_samples/ImageNet/combined_image_{}.jpg'.format(cn)) , combined_image)
